...
OpenAI Codex CLIを活用したスペック駆動開発フレームワークのテンプレートです。

ドキュメントファースト → 作業計画 → 実装 → 検証のサイクルを、Codex CLIのスキル・エージェントで自動化します。
...
```bash
cp -r codex-driven-template/ my-new-project/
cd my-new-project/
git init
```
...
### 4. AGENTS.md の技術スタックを更新

`$setup-project` 完了後、`AGENTS.md` の `## 技術スタック` セクションをプロジェクトに合わせて記載してください。

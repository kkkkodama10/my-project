...
既存リポジトリに新機能を追加するためのCodex駆動開発テンプレートです。
新規プロジェクト用ではなく、すでにコードベースが存在するリポジトリに `.codex/`、`.agents/` ディレクトリごとコピーして使います。
...
- **作業計画時**: `$steering` でモード1（ステアリングファイル作成）
- **実装時**: `$steering` でモード2（実装とtasklist.md更新管理）
- **検証時**: `$steering` でモード3（振り返り）

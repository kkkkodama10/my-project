---
name: add-feature
description: 新機能を既存パターンに従って、完全に無停止で実装する。`$add-feature [機能名]` で呼び出す。
---
...
2. `docs/`ディレクトリ内の該当機能の設計ドキュメントを確認し、関連する設計思想やアーキテクチャを理解する。

## ステップ3: 既存パターンの調査

1. 機能名に関連するキーワードでソースコードを検索する。
...
3. タスクの実行:
  ...
  - 既存コードのパターンと規約を常に遵守する。

4. タスクリストの更新:
  - 実行したタスクが完了したら、`tasklist.md`を更新し、該当タスクを `[ ]` から `[x]` に変更する。
...
- **ルールA: タスクが大きすぎる場合**
  - **対処法:** 現在のタスクをより小さな複数のサブタスクに分割する。元のタスクを削除し、その場所に新しいサブタスク（`[ ]`付き）を挿入する。その後、ループを継続する。

- **ルールB: 技術的理由でタスクが不要になった場合**
  - **条件:** 実装方針の変更、アーキテクチャの変更、依存関係の変更など、明確な技術的理由がある場合にのみ適用。
  - **対処法:** 該当タスクを `[x] ~~タスク名~~ (理由: [具体的な技術的理由を簡潔に記述])` の形式で更新する。その後、ループを継続する。
...
2. `implementation-validator`サブエージェントを起動して品質を検証する。
...
1. プロジェクトのテストコマンドを特定し（package.json, Makefile, pyproject.toml 等を確認）、テストを実行する。
...
2. 今回の変更が既存のアーキテクチャに影響を与えるか判断する。

3. 影響がある場合、`docs/`内の関連する設計ドキュメントを更新する。
